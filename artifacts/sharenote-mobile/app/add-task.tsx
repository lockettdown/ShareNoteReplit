...
function splitTaskTime(value: string) {
...
export default function AddTaskScreen() {
  ...
  const initialSelectedPersonIds = existingTask?.personIds?.length
    ...
    : [
        assignableMembers.find((member) => member.id === (existingTask?.personId ?? initialPersonId))?.id ??
      ...
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.restrictedContent, { paddingBottom: bottomPad + 32 }]}>
          <PermissionNotice
            ...
            message="This profile can view events and tasks, but cannot create or edit tasks."
      ...
      >
        <View style={[styles.card, { backgroundColor: colors.card, shadowColor: colors.shadow }]}>
          <View style={styles.fieldGroup}>
            <Text style={[styles.label, { color: colors.foreground, fontFamily: 'Inter_600SemiBold' }]}>Task Title</Text>
