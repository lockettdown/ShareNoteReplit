...
begin
  ...
  set family_email = excluded.family_email,
...
-- Remove the original UI prototype state if it was ever written to an account.
-- The full set of fixed profile IDs and names prevents matching a real family by name alone.
delete from public.family_states
where state ->> 'familyName' = 'Smith Family'
  and state -> 'members' @> '[{"id":"m1","name":"David Smith"},{"id":"m2","name":"Maya Smith"},{"id":"m3","name":"Leo Smith"}]'::jsonb
  and state -> 'dashboardMembers' @> '[{"id":"m4","name":"Lily Smith"}]'::jsonb;
